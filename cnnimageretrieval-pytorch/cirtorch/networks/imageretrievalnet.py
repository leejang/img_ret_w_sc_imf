...
def extract_ms(net, input, ms, msp):
    ...
    for s in ms: 
        if s == 1:
            ...
            #input_t = nn.functional.upsample(input, scale_factor=s, mode='bilinear', align_corners=False)
            input_t = LF.interpolate(input, scale_factor=s, mode='bilinear', align_corners=False)
