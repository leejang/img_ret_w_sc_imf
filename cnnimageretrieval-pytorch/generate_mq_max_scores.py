...
for idx, score in np.ndenumerate(scores_t):
  ...
  if (idx[0] % 5 == 4) and (idx[1] % (n_imgs) == (n_imgs - 1)):
    ...
    if (score > score_dict[idx[1]]):
      score_dict[idx[1]] = score

    for img_idx, m_score in score_dict.items():
      new_scores_t[m_idx * 5 + 0][img_idx] = m_score
      new_scores_t[m_idx * 5 + 1][img_idx] = m_score
      new_scores_t[m_idx * 5 + 2][img_idx] = m_score
      new_scores_t[m_idx * 5 + 3][img_idx] = m_score
      new_scores_t[m_idx * 5 + 4][img_idx] = m_score
